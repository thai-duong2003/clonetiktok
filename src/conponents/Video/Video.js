...
function Video({ data }) {
  ...
  function videoScroll() {
    const windowHeight = window.innerHeight; // lay rac hieu cao cua window
    const videoTag = videoref.current;

    const videoHeight = videoTag.clientHeight; //  lay rac hieu cao cua element
    const videoClientRect = videoTag.getBoundingClientRect().top; // tra ve kich thuoc cua phan tu va vij tri cua no so voikhung  nhin
  ...
  setconfig("ismute", true);
  console.log(config.ismute);
  return (
